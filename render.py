...
#         gt_image = (gt_image * alpha_mask + background[:, None, None] * (1.0 - alpha_mask)).clamp(0.0, 1.0) # [3, 267, 475]
#         depth_map = rendering_result["depth_map"] # [1, 267, 475]

#         depth_img = viridis_cmap(depth_map.squeeze().cpu().numpy())
#         depth_img = (depth_img * 255).astype(np.uint8) # [267, 475, 3]
...
def render_set(
    ...
    #     ssim_avg /= len(views)
    #     lpips_avg /= len(views)
    #     print(f"psnr_avg: {psnr_avg}; ssim_avg: {ssim_avg}; lpips_avg: {lpips_avg}")
