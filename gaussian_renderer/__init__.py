...
def render(
    ...
    try:
        screenspace_points.retain_grad() # gradient 추적할 수 있도록
    except:
        pass
    ...
    if pad_normal:
        ...
        normal_bg = torch.tensor([0.0, 0.0, 1.0], device=normal_map.device) # 이게 T? 
        normal_map = normal_map * opacity_map + (1.0 - opacity_map) * normal_bg[:, None, None] # gaussian에 저장된 normal map 계산하는 부분? 
    ...
    ### normal map -> normalize -> normal vector로 변환
